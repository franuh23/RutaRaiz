...
export default function PlanificadorPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = `/api/rutas/planificar?ruta_id=${selectedRuta}&localizacion_inicio_id=${inicioId}&km_dia=${kmDia}`;
    ...
    <Container>
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.formGroup}>
          ...
          <select value={inicioId} onChange={(e) => setInicioId(e.target.value)} required>
            ...
            {localizaciones.map(loc => (
              <option key={loc.id} value={loc.id}>{loc.nombre} ({loc.distancia_desde_inicio} km)</option>
            ))}
          </select>
        </div>
        ...
        <div className={styles.formGroup}>
          ...
          <input type="number" value={kmDia} onChange={(e) => setKmDia(e.target.value)} min="1" max="100" required />
        </div>
