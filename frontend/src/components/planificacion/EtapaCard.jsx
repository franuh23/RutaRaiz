...
export default function EtapaCard({ etapa }) {
  ...
  const { dia, inicio, fin, distancia, alojamientos = [] } = etapa || {};
  const tieneAlojamientos = alojamientos.length > 0;

  return (
    <div className="card shadow-sm border-0 mb-3 bg-white" style={{ borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
      <div 
      ...
      >
        ...
        <div className="d-flex align-items-center gap-2">
          <Badge variant="default" className="fs-6 px-3 py-2">{distancia} km</Badge>
        </div>
      </div>

      {tieneAlojamientos && isOpen && (
        <div className="card-body bg-light border-top p-3">
          <h6 className="text-uppercase text-muted fw-bold mb-3" style={{ fontSize: '0.75rem' }}>🏡 Dónde dormir en {fin}:</h6>
          <div className="row g-2">
            {alojamientos.map((aloj) => (
              <div className="col-12 col-md-6" key={aloj.id}>
                <div className="card border-0 bg-white p-3 h-100 d-flex flex-row justify-content-between align-items-center shadow-sm" style={{ borderRadius: 'var(--radius-md)' }}>
                  <div className="pe-2">
                    <Badge variant="difficulty-medium" size="sm" className="mb-1 text-uppercase">{aloj.tipo || 'Albergue'}</Badge>
